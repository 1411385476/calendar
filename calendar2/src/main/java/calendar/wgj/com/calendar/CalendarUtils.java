...
public class CalendarUtils {
    ...
    private void initAllHolidays(Context context){
        try {
            ...
            sAllHolidays = new Gson().fromJson(baos.toString(), new TypeToken<Map<String, int[]>>(){}.getType());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static String hasKey(int year, int month){
    ...
    public boolean removeTaskHints(int year, int month, int day){
        String key = hasKey(year, month);
        List<Integer> hints = sUtils.sMonthTaskHint.get(key);
        if(hints == null){
            hints = new ArrayList<>();
    ...
    public static int getMonthsAgo(int lastYear, int lastMonth, int year, int month){
        return (lastYear -year) * 12 + (lastMonth - month);
    }
    ...
    public static int getMonthRows(int year, int month){
        int size = getFirstDayWeek(year, month) + getMonthDays(year, month);
