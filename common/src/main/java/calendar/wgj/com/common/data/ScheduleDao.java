...
import calendar.wgj.com.common.bean.Schedule;
...
public class ScheduleDao {
    ...
    public int addSchedule(Schedule schedule){
        ...
        values.put(DBConfig.SCHEDULE_TIME, schedule.getTitle());
    ...
    private int getLastScheduleId(){
        ...
        Cursor cursor = db.query(DBConfig.SCHEDULE_TABLE_NAME, null, null, null, null, null, null );
        int id = 0;
    ...
    public List<Schedule> getScheduleByDate(int year, int month, int day){
        ...
        Cursor cursor = db.query(DBConfig.SCHEDULE_TABLE_NAME, null,
                String.format("%s=? and %s = ? and %s = ?", DBConfig.SCHEDULE_YEAR,
                        DBConfig.SCHEDULE_MONTH, DBConfig.SCHEDULE_DAY),
        ...
        while (cursor.moveToNext()){
            ...
            schedule.setTime(cursor.getInt(cursor.getColumnIndex(DBConfig.SCHEDULE_TIME)));
            schedule.setEventSetId(cursor.getInt(cursor.getColumnIndex(DBConfig.SCHEDULE_ID)));
    ...
    public List<Integer> getTaskHintByMomth(int year, int month){
        List<Integer> taskHint = new ArrayList<>();
        SQLiteDatabase db = mHelper.getReadableDatabase();
        Cursor cursor = db.query(DBConfig.SCHEDULE_TABLE_NAME, new String[]{DBConfig.SCHEDULE_DAY},
                String.format("%s=? and %s=?", DBConfig.SCHEDULE_YEAR, DBConfig.SCHEDULE_MONTH),
